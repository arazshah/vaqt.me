...
**آخرین به‌روزرسانی:** فاز ۱۱ — کیفیت و تحویل — تکمیل‌شده. تست E2E مسیر
طلایی (Playwright، در CI)، `Dockerfile` چندمرحله‌ای production برای هر دو
اپ، و بازبینی امنیتی (helmet، Swagger production-gated، ۳۳→۲ آسیب‌پذیری
وابستگی رفع‌شده) همگی merge شدند
...
callback ایدمپوتنت + آشتی‌سازی + آداپتر mock/Zarinpal) و UI (صفحه‌ی
نتیجه‌ی پرداخت + دکمه‌های خرید محصول + `/pricing`) هر دو merge شدند؛
فاز ۱۰ (تکمیل تجربه) کامل — نظرات/امتیازدهی، پروفایل عمومی (`/users/:id`)،
PWA (manifest + آیکون‌ها + theme-color)، و SEO صفحه‌ی درخواست
(metadata/JSON-LD/sitemap/robots/OG Image داینامیک) همگی merge شدند
...
پس از هر بازبینی زنده‌ای که کاربران/درخواست‌های تستی در Postgres/Redis واقعی dev می‌سازد، از اسکریپت `pnpm --filter @vaqt/api cleanup:qa` برای پاک‌سازی امن استفاده کنید — جزئیات کامل (معیار تشخیص، لایه‌های ایمنی، باگ‌های کشف‌شده) در skill `cleanup-qa-data`.
...
| ~~`ts-node --transpile-only` موقتی است~~                                                                                                                                                                       | ✅ رفع شد — `packages/shared` و `packages/db` با `tsup` به `dist` بیلد می‌شوند، `apps/api` به `nest start --watch` بازگشت.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | —        |
...
| ~~`@vaqt/db#typecheck` و `lint:fix` بدون `^build` (تکرار همان الگوی باگ)~~                                                                                                                                     | ✅ رفع شد در فاز ۴ — override اختصاصی یک تسک برای یک پکیج، config عمومی همان تسک را جایگزین می‌کند نه merge؛ `^build` به `@vaqt/db#typecheck`/`lint:fix`/`seed` اضافه شد.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | —        |
| ~~`generate` تسک turbo بدون `inputs`/`outputs` واقعی — cache می‌توانست client گم‌شده را «موفق» نشان دهد~~                                                                                                      | ✅ رفع شد در فاز ۴ — بدون `output` سفارشی روی Prisma، turbo نمی‌توانست کلاینت گم‌شده را تشخیص دهد. رفع: `output = "../generated/prisma"` در schema.prisma + `inputs`/`outputs` صریح در turbo.json (کش واقعی فعال شد).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | —        |
| ~~Race ۱ — `@vaqt/db#build` بدون `^build` (پنجمین شاهد الگوی تکرارشونده)~~                                                                                                                                     | ✅ رفع‌شده در کامیت `76235127c9dc11163045bf804d8c126b7c462e3a` (داخل PR #۱۱، بدون بازبینی مستقل جدا).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | —        |
...
| ~~چهار GitHub Action در `ci.yml` هدف Node.js 20 دارند~~                                                                                                                                                        | ✅ رفع شد (۲۰۲۶-۰۹-۰۷، PR مستقل): `actions/checkout@v4→v7.0.1`، `actions/cache@v4→v6.1.0`، `actions/setup-node@v4→v7.0.0`، `pnpm/action-setup@v4→v6.1.0` — نسخه‌ی واقعی هر چهار تگ دوباره با `gh api repos/<owner>/<action>/releases/latest` در لحظه‌ی ارتقا تأیید شد (نه اعداد حدسی بازبینی قبلی؛ `pnpm/action-setup` از `v6.0.10` حدسی‌شده به `v6.1.0` واقعی رسیده بود). هر ۸ رخداد (۴ اکشن × ۲ job در `ci.yml`) به‌روزرسانی شدند؛ `actions/upload-artifact@v4` عمداً دست‌نخورده ماند چون در annotation اصلی فهرست نشده بود. **اثبات زنده:** هر دو job (`ci`, `e2e`) روی یک اجرای واقعی GitHub Actions سبز شدند، بدون annotation جدید.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | —        |
| ~~نقض قاعده‌ی RPC-only (فقط `@Get`/`@Post`) در `users.controller.ts`~~                                                                                                                                         | ✅ رفع شد (بازبینی post-فاز-۱۱): `PATCH /users/me` → `POST /users/me/update`، `PUT /users/me/skills` → `POST /users/me/skills/put`، `DELETE /users/me/avatar` → `POST /users/me/avatar/delete` — دقیقاً هم‌الگو با `OffersController`/`RequestsController` (فعل در انتهای مسیر). متدهای کنترلر و منطق سرویس دست‌نخورده ماندند؛ فقط دکوریتور HTTP و مسیر عوض شد. کالرهای واقعی این مسیرها (`users.body-validation.e2e.spec.ts`، `apps/web/e2e/fixtures/api-helpers.ts`) به‌روزرسانی شدند؛ کد وضعیت انتظاری این دو تست از ۲۰۰ به ۲۰۱ اصلاح شد (رفتار پیش‌فرض Nest برای `@Post`). اثبات: `users.controller.spec.ts` + `users.body-validation.e2e.spec.ts` (روی Postgres/Redis واقعی) + `public-routes.spec.ts` + `origin-check.middleware.spec.ts` — هر ۲۴ تست سبز.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | —        |
...
| ~~`AllExceptionsFilter` هر `HttpException` غیر-`AppError` را بی‌قیدوشرط `VALIDATION_ERROR` می‌گذارد و فقط `body.message` را می‌خواند، نه `body.errors`~~                                                       | ✅ رفع شد (بازبینی post-فاز-۱۱): فیلتر اکنون `body.errors` (فهرست کامل zod issues) را بر `body.message` عمومی ترجیح می‌دهد و به‌عنوان `details` پاس می‌دهد؛ کد خطا دیگر همیشه `VALIDATION_ERROR` نیست — با نگاشت صریح `status → ErrorCode` (۴۰۰→VALIDATION_ERROR، ۴۰۱→UNAUTHORIZED، ۴۰۳→FORBIDDEN، ۴۰۴→NOT_FOUND، پیش‌فرض VALIDATION_ERROR) انتخاب می‌شود. دو تست جدید در `all-exceptions.filter.spec.ts` اضافه شد؛ اثبات: ۱۰/۱۰ تست سبز.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | —        |
| ~~`nav` داخل `AppShell` بدون فاصله‌ی واقعی بین آیتم‌ها رندر می‌شود — کلاس `gap-4` روی عنصر اعمال نمی‌شود~~                                                                                                     | ✅ رفع شد — ریشه‌یابی کامل شد و معلوم شد دامنه‌ی واقعی باگ خیلی بزرگ‌تر از فقط `nav` بود: توکن پایه‌ی `--spacing` تیلویند v4 (که هر یوتیلیتی مقیاس‌دار — `p-*`, `m-*`, `gap-*`, `w-*`, `h-*`, `max-w-*`, `inset-*`, `translate-*` — را با `calc()` از روی آن می‌سازد) اصلاً در خروجی کامپایل‌شده وجود نداشت؛ پل `@config` به `tailwind.config.ts` قدیمی فقط `theme.extend.spacing` به سبک v3 را بارگذاری می‌کند و مقدار پیش‌فرض `--spacing` خودِ v4 را جایگزین نمی‌سازد. نتیجه: هر یوتیلیتی مقیاس‌دار در **کل اپ** بدون هیچ اثری در DOM حاضر بود ولی صفر CSS تولید می‌کرد (اثبات مستقیم: `grep` روی CSS کامپایل‌شده برای `px-4`/`gap-4`/`space-y-4`/... صفر match، در حالی که `flex`/`grid`/`rounded-xl` بدون مقیاس درست تولید می‌شدند). رفع با یک خط: `--spacing: 0.25rem;` در بلوک `@theme inline` در `packages/ui/src/styles/globals.css`. اثبات زنده: `getComputedStyle(nav).gap` روی `/requests` اکنون `16px` (دقیقاً `1rem`)؛ اسکرین‌شات کامل صفحه‌ی اصلی و `/requests` قبل/بعد مقایسه شد.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | —        |
...
| `@nestjs/core@10.4.22` یک آسیب‌پذیری moderate («Improperly Neutralizes Special Elements in Output Used by a Downstream Component») دارد که فقط در NestJS ۱۱ (`>=11.1.18`) رفع شده، نه یک نسخه‌ی patch روی ۱۰.x | کشف‌شده در بازبینی امنیتی فاز ۱۱ (`pnpm audit`). ارتقا به NestJS ۱۱ یک تغییر عمده‌ی هماهنگ روی حدود ۹ پکیج `@nestjs/*` + سازگاری `nestjs-zod`/`@nest-zod/z` است — عمداً در همین PR انجام نشد (بر خلاف multer که ارتقای آن با NestJS ۱۰ فعلی سازگار از آب درآمد). همچنین `esbuild<0.28.1` یک آسیب‌پذیری low (خواندن فایل دلخواه، فقط وقتی dev server آن روی Windows expose شود) دارد که چون این پروژه نه از dev server خودِ esbuild به‌صورت مستقیم/عمومی استفاده می‌کند و نه روی Windows دیپلوی می‌شود، ریسک عملی ندارد و عمداً رفع نشد.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | PR جدا   |
...
- ~~صفحه‌ی اصلی هنوز یک لندینگ واقعی نیست.~~ ✅ رفع شد (بازبینی
  ...
  از کارت نمایشی فاز ۴ («فاز ۴ — سیستم طراحی»، دکمه‌ی بدون `href`) به یک
  لندینگ واقعی تبدیل شد: hero با عنوان/زیرعنوان واقعی (`fa.homePage.*`،
  نه رشته‌ی hardcode)، دو CTA با لینک واقعی (`مشاهده‌ی درخواست‌ها` → `/requests`؛
  دومی بسته به `useAuth()` بین `ورود و شروع` → `/login` و
  `ثبت درخواست جدید` → `/requests/new` سوییچ می‌کند)، و سه کارت ویژگی
  (`lucide-react` برای آیکون — وابستگی جدید `apps/web`). صفحه از
  `Card`/`Button` موجود `@vaqt/ui` و `AppShell` موجود استفاده می‌کند، بدون
  کامپوننت جدید. متادیتای سطح سایت (`layout.tsx`) از قبل درست بود، بدون
  تغییر.

  **اثبات زنده (Playwright، هر دو حالت کاربر):** با سرور dev واقعی (API
  روی ۳۰۰۱، web موقتاً روی ۳۱۰۰ به‌خاطر تداخل پورت ۳۰۰۰ با یک پروژه‌ی
  دیگر روی همین ماشین؛ `WEB_ORIGIN` بک‌اند موقتاً به همان پورت تنظیم و
  بعد برگردانده شد) — حالت مهمان: هر دو دکمه با متن/لینک درست، سه کارت
  ویژگی رندر شده، بدون خطای کنسول به‌جز دو `401` مورد‌انتظار `GET /auth/me`
  (همان الگوی مستندشده‌ی قبلی). حالت کاربر واردشده (OTP واقعی با
  `DEV_FIXED_OTP`): دکمه‌ی دوم به‌درستی «ثبت درخواست جدید» شد. کاربر
  تستی و ردیف‌های مرتبط (`sessions`, `verification_codes`, `audit_logs`)
  بعداً مستقیم از Postgres واقعی حذف شدند (چون شماره‌ی تصادفی این تست با
  `TEST_PHONE_PREFIX` اسکریپت `cleanup:qa` مطابقت نداشت)؛ شمارش پایه‌ی
  seed تأیید شد بدون تغییر ماند: ۸ کاربر.

  `pnpm --filter @vaqt/web typecheck/lint/build` سبز (بودجه‌ی باندل:
  `/page: 159.8 KB`، در حد مجاز ۲۳۰ KB).
...
| ۱۱ — کیفیت و تحویل | ✅ تکمیل‌شده    | E2E (golden path، در CI)، Dockerfile چندمرحله‌ای production، و بازبینی امنیتی — هر سه merge شدند                                   |
...
### بازبینی UI/UX — فاز صفر بازطراحی حرفه‌ای (شروع‌شده)

کاربر UI فعلی را «زشت و غیرحرفه‌ای» ارزیابی کرد — تأیید بصری با اسکرین‌شات واقعی
Playwright از صفحه‌ی اصلی/فهرست درخواست‌ها/ورود: پالت کاملاً grayscale (هر توکن رنگ
`oklch(... 0 0)`، بدون chroma)، کارت‌های بدون سایه با فقط border نازک، nav با
underline پیش‌فرض مرورگر، و فهرست درخواست‌ها با کلاس `grid gap-4 sm:grid-cols-2
lg:grid-cols-3` که در عمل تک‌ستونه رندر می‌شد. یک سند برنامه‌ی بازطراحی کامل
(اصول طراحی، ارتقای سیستم طراحی، بازطراحی صفحه‌به‌صفحه، نقشه‌راه فازبندی‌شده در
۶ فاز) ساخته و با کاربر به اشتراک گذاشته شد؛ این بخش فاز ۰ آن نقشه‌راه را
مستند می‌کند (توکن‌ها + کامپوننت‌های پایه)، نه بازطراحی صفحه‌به‌صفحه که فازهای
بعدی‌اند.

**باگ واقعی کشف‌شده حین کار (نه فقط کمبود طراحی):** همان الگوی دقیق حادثه‌ی
`--spacing` بالا، این‌بار برای breakpointها. `@theme inline` در `globals.css`
هیچ `--breakpoint-*`ای تعریف نمی‌کرد و پل `@config` به `tailwind.config.ts`
قدیمی (که خودش هم `screens` را override نمی‌کند) این توکن‌های پیش‌فرض v4 را
سینتز نمی‌کند — یعنی **هر** utility پاسخگو (`sm:`/`md:`/`lg:`/`xl:`/`2xl:`) در
کل اپ، از ابتدای فاز ۴، هیچ CSSای تولید نمی‌کرد (کلاس در DOM حاضر، بدون اثر؛
دقیقاً همان علامت باگ `--spacing`). **اثبات زنده (قبل/بعد):** یک اسکریپت
Playwright مستقیماً `getComputedStyle` را روی `.grid` فهرست درخواست‌ها در
عرض ۱۴۴۰px خواند — قبل از رفع: `gridTemplateColumns: "1424px"` (یک ستون)؛ بعد
از افزودن `--breakpoint-sm/md/lg/xl/2xl` با مقادیر پیش‌فرض خودِ v4 به `@theme
inline`: `gridTemplateColumns: "464px 464px 464px"` (سه ستون واقعی). چون این
باگ صفحه‌ی اصلی (کارت‌های ویژگی با `md:grid-cols-3` مشابه) را هم می‌گرفت،
رفعش فوراً چند صفحه را هم‌زمان درست کرد، نه فقط فهرست درخواست‌ها.

**تغییرات این فاز:**

- `packages/ui/src/styles/globals.css`: پالت رنگ برند واقعی جایگزین grayscale
  شد — یک هیوی ایندیگو/بنفش (۲۷۵) روی همه‌ی توکن‌های معنایی (`primary`,
  `secondary`, `accent`, `muted`, `border`, `ring`, `chart-*`) در هر دو حالت
  light/dark، به‌علاوه‌ی یک مقیاس سایه‌ی جدید (`--shadow-sm/md/lg/xl`، رنگی نه
  خاکستری خالص) و افزایش `--radius` پایه از `0.625rem` به `0.75rem`. هر
  جفت متن/پس‌زمینه‌ی موجود در `globals.contrast.test.ts` (۱۰ جفت × ۲ تم) با
  یک اسکریپت جدا از روی همان تابع `contrastRatio`ی واقعی پروژه از قبل بررسی
  و ≥۴٫۵:۱ تأیید شد، سپس خودِ تست اجرا و سبز شد (۲۰/۲۰) — بدون حدس‌زدن رنگ.
- `packages/ui/src/components/container.tsx`, `section.tsx` (جدید): دو
  primitive layout — `Container` (max-width + gutter یکسان، جایگزین
  `mx-auto w-full max-w-5xl px-4` تکرارشونده) و `Section` (مقیاس فاصله‌ی
  عمودی بین بخش‌های صفحه) — دستی نوشته شدند، نه با `shadcn add` (مثل
  `price-tag.tsx`/`bidi-input.tsx` موجود، مصرف با deep-import).
- `packages/ui/src/components/ui/card.tsx`: افزودن `shadow-sm` + انتقال نرم
  (`transition-shadow`) — قبلاً فقط یک `ring-1` نازک بود.
- `packages/ui/src/components/ui/button.tsx`: افزودن `no-underline` به کلاس
  پایه — قبلاً وقتی `asChild` یک `<Link>` را می‌پیچید، underline پیش‌فرض
  مرورگر روی هر دکمه‌ی لینک‌شده (مثل CTAهای صفحه‌ی اصلی) دیده می‌شد.
- `apps/web/src/components/app-shell.tsx`: هدر `sticky` با `backdrop-blur`،
  استفاده از `Container` جدید به‌جای max-width تکرارشونده، و کلاس
  `no-underline` روی لینک‌های nav (که تا امروز underline پیش‌فرض مرورگر
  داشتند — به اسکرین‌شات اولیه‌ی این بازبینی مراجعه شود).

**اثبات زنده:** `pnpm lint && pnpm typecheck && pnpm build && pnpm test` کامل
مونوریپو سبز (۸/۸/۵/۸ تسک، ۴۰۲/۴۰۲ تست `apps/api` بدون تغییر، بودجه‌ی باندل
`apps/web` سبز: `/page: 160.6 KB`). اسکرین‌شات‌های قبل/بعد از صفحه‌ی اصلی و
فهرست درخواست‌ها (Playwright، سرور dev واقعی) گرفته و مقایسه شد.

**باقی‌مانده (فازهای ۱ تا ۵ نقشه‌راه):** بازطراحی خودِ صفحات (Hero واقعی،
صفحه‌ی ورود split-layout، فرم‌ها، چت، پروفایل، pricing) — این فاز فقط
زیرساخت (توکن‌ها + primitiveها + رفع باگ breakpoint) بود، نه بازطراحی
صفحه‌به‌صفحه.

### فاز ۱ نقشه‌راه — Hero واقعی + صفحه‌ی ورود (PR جدا، روی فاز ۰ سوار)

بلافاصله بعد از فاز ۰ ادامه یافت (طبق مجوز کار خودمختار موجود). دامنه‌ی این
PR عمداً به دو صفحه محدود شد تا کوچک و قابل‌بازبینی بماند:

- **صفحه‌ی اصلی:** بج eyebrow، پس‌زمینه‌ی gradient نرم (`radial-gradient`
  ساخته‌شده از خودِ توکن `--primary`، بدون هیچ تصویر/CDN — طبق بند ۲
  همین فایل)، heading بزرگ‌تر و پررنگ‌تر، کارت‌های ویژگی با آیکون داخل
  دایره‌ی رنگی + سایه‌ی hover. کلاس‌های قدیمی `text-brand-900`/
  `text-text-muted` (رنگ‌های هاردکد فاز ۰ قدیم) با توکن‌های معنایی
  `text-foreground`/`text-muted-foreground` جایگزین شدند.
- **صفحه‌ی ورود:** کارت به‌صورت عمودی در مرکز صفحه، با یک بج آیکون
  (`KeyRound`) و زیرعنوان؛ خودِ `OtpForm` دست‌نخورده ماند. split-layout
  کامل (نیمه‌ی تصویری/برندی) عمداً به فاز بعد موکول شد — چون `AppShell`
  محتوا را همیشه داخل `Container` با padding می‌پیچد و یک split-layout
  واقعی نیاز به یک مسیر خروج از آن wrapper دارد؛ آن تغییر معماری‌تر است.
- دو کلید پیام جدید (`homePage.heroEyebrow`, `loginPage.subtitle`) به
  `apps/web/messages/fa.ts` اضافه شدند — طبق قاعده‌ی «بدون رشته‌ی فارسی
  هاردکد».

**اثبات:** `pnpm --filter @vaqt/web typecheck/lint/build` سبز
(`/page: 161.0 KB`، در حد بودجه)؛ اسکرین‌شات زنده‌ی هر دو صفحه روی سرور dev
واقعی گرفته شد.

### فاز ۲ نقشه‌راه — فهرست + جزئیات (فیلتربار، grid ریسپانسیو، صفحه‌ی جزئیات دوستونه)

بلافاصله بعد از merge شدن فاز ۱ ادامه یافت (طبق مجوز کار خودمختار). این فاز
هم‌زمان دو شکاف را می‌بندد: بازطراحی بصری نقشه‌راه (grid/فیلتربار/دوستونه)
و بدهی محصولی مستندشده‌ی فاز ۵ («فیلترهای فهرست... این برش عمداً بدون
فیلتر ساخته شد») — چون فیلتربار واقعی بدون فیلتر واقعی سمت سرور بی‌معنی
بود، این PR هر دو طرف را با هم پیاده کرد، نه فقط CSS.

- **بک‌اند:** `listRequestsSchema` (`packages/shared`) چهار فیلتر اختیاری
  گرفت — `categoryId`, `mode`, `city` (تطابق دقیق، نه substring — طبق بند
  ۶ همین فایل که فقط بودجه را صریحاً استثنا می‌کند ولی همان احتیاط برای
  فیلترهای دیگر هم رعایت شد)، و `search` (متن آزاد، روی همان ستون
  `searchText` نرمال‌شده با `normalizeFa` که فاز ۱ برایش ایندکس GIN
  trigram ساخته بود — بند ۱۶). هر چهار در `RequestsService.list()` با AND
  به `where` اضافه می‌شوند، بدون دست‌زدن به منطق cursor/tie-break موجود.
  ۶ تست جدید روی Postgres واقعی (هرکدام فیلتر جدا + یک تست AND ترکیبی)،
  به‌علاوه‌ی ۵ تست schema جدید در `packages/shared`.
- **UI:** کامپوننت جدید `RequestsFilterBar` (جست‌وجو با آیکون + دیبانس
  ۴۰۰ میلی‌ثانیه‌ای محلی، Select دسته/حالت، Input شهر) روی `/requests`؛
  grid فهرست با hover واقعی (`translate-y` + `shadow-md` روی خودِ کارت،
  نه فقط لینک بی‌اثر قبلی). صفحه‌ی جزئیات (`/requests/[id]`) به یک
  `grid lg:grid-cols-3` تبدیل شد: محتوای اصلی (`lg:col-span-2`) در یک
  کارت با border/shadow، و ستون کناری (بالا/پرداخت + پیشنهادها) کنارش —
  به‌جای پشته‌ی عمودی قبلی.

**باگ واقعی کشف‌شده حین اثبات زنده (نه با typecheck/lint/build که هر سه
از ابتدا سبز بودند):** `RequestsFilterBar` برای پرکردن Select دسته، به
`GET /categories` وصل می‌شود — که (بند ۲۹) نیازمند ورود است، نه
`@Public()`. فراخوانی اولیه‌ی این کامپوننت با `apiFetch` بدون
`redirectOnAuthFailure: false` نوشته شده بود؛ چون پیش‌فرض این پرچم `true`
است (`api-client.ts`)، هر بازدیدکننده‌ی مهمانِ صفحه‌ی کاملاً عمومی
`/requests` با ۴۰۱ همین فراخوانی فوراً به `/login` پرتاب می‌شد — یک
رگرسیون واقعی روی مسیر مهمان که فاز ۵ عمداً برایش تست منفی نوشته بود.
کشف شد با یک اسکریپت Playwright headless که بعد از `goto('/requests')`
`page.url()` نهایی را چک می‌کرد: قبل از رفع، مقدار نهایی `/login` بود؛
بعد از افزودن `redirectOnAuthFailure: false` (و catch به آرایه‌ی خالی،
هم‌الگو با `request-form.tsx`)، `page.url()` روی `/requests` باقی ماند و
مهمان فقط گزینه‌ی «همه‌ی دسته‌ها» را می‌بیند (فیلتر دسته برای مهمان عملاً
غیرفعال می‌ماند — محدودیت شناخته‌شده، نه یک رفع کامل؛ چون `/categories`
عمداً نیازمند ورود طراحی شده، باز کردن آن به کاربران مهمان یک تصمیم
محصولی جداست که در این PR گرفته نشد).

**اثبات زنده (Playwright headless + curl، Postgres/Redis واقعی، سرور dev
واقعی):**

- فیلتر `mode=IN_PERSON` روی seed واقعی → دقیقاً همان یک درخواست غیرآنلاین
  seed برگشت.
- فیلتر `search=پایتون` → دقیقاً «مشاوره کدنویسی پایتون» (هم در curl مستقیم
  API و هم در مرورگر واقعی بعد از تایپ در جعبه‌ی جست‌وجو با دیبانس).
- `mode` نامعتبر (`NOPE`) → `400 VALIDATION_ERROR` با جزئیات zod، نه ۵۰۰.
- ورود واقعی با OTP (`DEV_FIXED_OTP`)، تزریق کوکی‌های واقعی در Playwright،
  و مشاهده‌ی صفحه‌ی جزئیات دوستونه‌ی واقعی (`req-python-consult`) — عنوان،
  بج وضعیت، `PriceTag` تومانی، مهلت با تقویم فارسی، و پنل کناری (راهنمای
  تکمیل پروفایل چون کاربر تستی مهارت/بیو نداشت) همگی درست رندر شدند.
- داده‌ی تستی (۱ کاربر + نشست‌ها/کدهای تأیید مرتبط) بعداً مستقیم از
  Postgres واقعی حذف شد؛ شمارش پایه‌ی seed تأیید شد بدون تغییر ماند: ۸
  کاربر، ۱۵ درخواست، ۲۰ پیشنهاد، ۲ گفتگو.

`pnpm --filter @vaqt/api typecheck/lint`، `pnpm --filter @vaqt/web
typecheck/lint/build` (بودجه‌ی باندل سبز، `/requests: 172 KB`)، و تست‌های
`requests.service.spec.ts` (۲۷/۲۷، شامل ۶ تست جدید فیلتر) و
`packages/shared` (۱۵۸/۱۵۸) همگی سبز.

**بازبینی Codex بعد از باز شدن PR (فاز ۲) — دو یافته‌ی واقعی، هر دو رفع
شدند:**

1. **Race واقعی بین درخواست‌های فیلتر پیاپی**: `loadFirstPage`/
   `handleLoadMore` بدون هیچ چک ترتیب، همیشه آخرین پاسخی که از شبکه
   برمی‌گشت را می‌نشاندند — نه لزوماً پاسخِ آخرین فیلتری که کاربر انتخاب
   کرده بود. اگر یک پاسخ کند برای فیلتر قدیمی بعد از یک پاسخ سریع برای
   فیلتر جدید برسد، نتیجه‌ی قدیمی نمایش داده می‌شد. رفع با یک شمارنده‌ی
   نسل (`requestGeneration` ref) که هر فراخوانی جدید افزایش می‌دهد؛ پاسخی
   که نسلش با نسل فعلی مطابقت نداشته باشد، بی‌صدا دور انداخته می‌شود.
   **اثبات زنده:** یک اسکریپت Playwright با `page.route` عمداً پاسخ اولین
   جست‌وجو را ۱٬۵۰۰ میلی‌ثانیه کند کرد و بلافاصله فیلتر را به عبارت دومی
   تغییر داد — بدون رفع، نتیجه‌ی نهایی می‌بایست فیلتر اول (قدیمی) را نشان
   می‌داد؛ با رفع، دقیقاً نتیجه‌ی فیلتر دوم (جدید) ماند.
2. **طول ورودی جست‌وجو/شهر بدون محدودیت سمت کلاینت**: `listRequestsSchema`
   `search` را حداکثر ۱۲۰ کاراکتر و `city` را حداکثر ۸۰ کاراکتر می‌پذیرد،
   ولی `Input`های فرانت‌اند هیچ `maxLength`ای نداشتند — یعنی ورودی
   طولانی‌تر با `400` رد می‌شد و صفحه آن را به‌اشتباه به‌عنوان خطای عمومی
   اتصال نشان می‌داد. رفع با افزودن `maxLength={120}`/`maxLength={80}`
   دقیقاً برابر همان اعداد schema. **اثبات زنده:** پرکردن جعبه‌ی جست‌وجو
   با ۲۰۰ کاراکتر → مقدار واقعی ورودی در مرورگر دقیقاً ۱۲۰ کاراکتر ماند؛
   همان تست برای شهر با ۸۰.

هر دو thread بازبینی روی PR resolve شدند؛ commit رفع (`851b31f`) با CI
سبز دوباره merge شد.

### فاز ۳ نقشه‌راه — فرم‌ها + چت

بلافاصله بعد از merge شدن فاز ۲ ادامه یافت (طبق مجوز کار خودمختار). دامنه‌ی
این PR عمداً کاملاً بصری ماند — هیچ منطق فرم/چت (react-hook-form، اعتبارسنجی
zod، Socket.IO) دست نخورد، فقط چیدمان و استایل:

- **صفحات `/requests/new` و `/requests/new/ai`:** هدر یکسان با الگوی
  صفحه‌ی ورود فاز ۱ — بج آیکون دایره‌ای (`FileEdit`/`Sparkles` از
  `lucide-react`)، عنوان، زیرعنوان جدید (`newRequestPage.subtitle`،
  `aiWizardPage.subtitle`)، و لینک تعویض دستی/AI که از یک متن ساده‌ی
  آبی به یک بج pill (border + `no-underline` + hover) تبدیل شد.
- **`RequestForm`:** کل فرم اکنون داخل یک `Card` است (قبلاً بدون هیچ
  wrapper، مستقیم روی پس‌زمینه‌ی صفحه شناور بود)، با چهار بخش
  جداشده با `border-t` + عنوان کوچک (`newRequestPage.sections.*`):
  اطلاعات پایه / جزئیات همکاری / بودجه / زمان‌بندی. ردیف‌های
  `preferredWindows` (که قبلاً فقط `flex` ساده بودند) هرکدام یک باکس
  `rounded-lg border bg-muted/40` گرفتند. کارت پیش‌نمایش (بعد از ساخت
  DRAFT) هم یک آیکون `FileCheck` در هدر گرفت، هم‌الگو با بقیه‌ی کارت‌های
  آیکون‌دار پروژه (لاگین، صفحه‌ی جزئیات).
- **`AiWizard`:** ناحیه‌ی پیام‌های گفت‌وگو یک `max-h-80 overflow-y-auto`
  گرفت — قبلاً بدون سقف بود و یک گفت‌وگوی طولانی کل صفحه را می‌کشید.
- **`/conversations` (فهرست گفتگوها):** هر کارت اکنون یک `Avatar` واقعی
  (حرف اول نام طرف مقابل) دارد؛ لینک‌ها `no-underline` شدند و کارت‌ها
  `hover:shadow-md` گرفتند (هم‌الگو با فهرست درخواست‌ها در فاز ۲).
- **`/conversations/[id]`:** هدر گفتگو به یک کارت واقعی (`border` +
  `shadow-sm`) با `Avatar` طرف مقابل تبدیل شد؛ خودِ `MessageThread` هم
  داخل یک کارت مشابه قرار گرفت. `MessageThread` یک سقف ارتفاع
  (`max-h-[28rem] overflow-y-auto`) گرفت تا thread طولانی یک پنجره‌ی
  چت واقعی باشد، نه صفحه‌ای که تا بی‌نهایت پایین می‌رود.

**اثبات زنده (Playwright headless + curl، Postgres/Redis واقعی، سرور dev
واقعی، دو کاربر واقعی seeker/provider با OTP واقعی):** جریان کامل
ساخت‌درخواست → پیشنهاد → انتخاب → گفتگوی واقعی → دو پیام رفت‌وبرگشتی
از طریق API واقعی اجرا شد. اسکرین‌شات‌های واقعی گرفته‌شده تأیید کردند:
فرم ساخت درخواست با چهار بخش جداشده و هدر آیکون‌دار درست رندر می‌شود؛
ویزارد AI با همان الگوی هدر و یک pill سوییچ به فرم دستی؛ فهرست گفتگوها
با آواتار/بج وضعیت/hover؛ صفحه‌ی جزئیات گفتگو با هدر کارتی + آواتار،
حباب‌های پیام درست‌ترازشده (پیام «من» در انتهای منطقی/چپ بصری RTL، طبق
تصمیم مستندشده‌ی فاز ۸)، و پنل نظر زیر آن — همه بدون هیچ خطای کنسول.
داده‌ی تستی (۲ کاربر، ۱ درخواست، ۱ پیشنهاد، ۱ گفتگو، ۲ پیام) بعداً
مستقیم از Postgres واقعی پاک شد؛ شمارش پایه‌ی seed تأیید شد بدون تغییر
ماند: ۸ کاربر، ۱۵ درخواست، ۲۰ پیشنهاد، ۲ گفتگو، ۲ نظر.

`pnpm --filter @vaqt/web typecheck/lint/build` سبز (بودجه‌ی باندل سبز)؛
تست‌های بک‌اند مرتبط (`requests`/`conversations`/`reviews`، ۵۴/۵۴) بدون
تغییر ماندند چون این PR هیچ منطق سرور را لمس نکرد — فقط UI.

**بازبینی Codex بعد از باز شدن PR (فاز ۳) — یک یافته‌ی واقعی، رفع شد:**
ناحیه‌ی پیام‌های `AiWizard` یک `max-h-80 overflow-y-auto` گرفته بود ولی
هیچ auto-scroll نداشت — وقتی گفت‌وگو از سقف ارتفاع بگذرد، پاسخ تازه زیر
ناحیه‌ی دیده‌شده رندر می‌شد و تا اسکرول دستی «گم» به‌نظر می‌رسید. رفع با
همان الگوی `bottomRef` که `MessageThread` از قبل داشت. **اثبات زنده:**
یک اسکریپت Playwright چهار نوبت پیام رد‌و‌بدل کرد تا واقعاً از ارتفاع
سقف بگذرد (`scrollHeight: 656` در برابر `clientHeight: 328`)؛ بدون رفع
`scrollTop` صفر می‌ماند، با رفع `scrollTop: 325` (تقریباً حداکثر) شد و
آخرین حباب کاملاً داخل ناحیه‌ی دیده‌شده افتاد.

### فاز ۴ نقشه‌راه — پروفایل + پرداخت

بلافاصله بعد از merge شدن فاز ۳ ادامه یافت (طبق مجوز کار خودمختار). دامنه‌ی
این PR هم کاملاً بصری ماند — هیچ endpoint یا منطق پرداخت/پروفایل دست
نخورد، فقط سه صفحه‌ای که نقشه‌راه برایشان معین کرده بود (پروفایل عمومی،
`/pricing`، صفحه‌ی نتیجه‌ی پرداخت):

- **`/users/[id]` (پروفایل عمومی):** بخش هدر (آواتار + نام + headline +
  شهر + امتیاز + بیو + مهارت‌ها) داخل یک کارت واقعی (`border` +
  `shadow-sm`) قرار گرفت — قبلاً مستقیم روی پس‌زمینه‌ی صفحه شناور بود.
  آواتار یک `ring-2 ring-primary/15` گرفت. عنوان بخش نظرات
  (`MessageSquareText` از `lucide-react`) آیکون گرفت، هم‌الگو با بخش‌های
  دیگر (اطلاعات پایه‌ی فرم درخواست در فاز ۳).
- **`/pricing`:** هدر یک بج آیکون دایره‌ای (`Crown`) گرفت، هم‌الگو با
  صفحات فاز ۱/۳. هر کارت محصول اکنون یک آیکون اختصاصی در `CardHeader`
  دارد — نگاشت `PRODUCT_ICONS` بر اساس `ProductCode` (`Crown` برای
  `PRO_MONTHLY`، `Bell` برای `TARGETED_NOTIFY`، `Sparkles` برای سه محصول
  سطح-درخواست که این صفحه فیلترشان می‌کند ولی برای کامل‌بودن type-map
  همچنان تعریف شدند) — و `hover:shadow-md`.
  محصولات همان endpoint (`GET /payments/products`) و همان فیلتر
  `ACCOUNT_LEVEL_CODES` قبلی را دارند؛ فقط استایل عوض شد.
- **`/payment/result`:** آیکون وضعیت (`CheckCircle2`/`XCircle`/
  `HelpCircle` بر اساس `status=success|failed|not_found`) با رنگ متناظر
  (`primary`/`destructive`/`muted`) داخل `EmptyMedia` قرار گرفت — قبلاً
  فقط متن بدون هیچ نشانه‌ی بصری بود. جدول جزئیات سفارش از یک `dl` با
  `border` خام به یک کارت واقعی (`rounded-xl border bg-card shadow-sm`)
  ارتقا یافت، هم‌شکل بقیه‌ی کارت‌های پروژه.

**اثبات زنده (Playwright headless + curl، Postgres/Redis واقعی، سرور dev
واقعی، یک کاربر واقعی با OTP واقعی):** پروفایل کاربر با `PATCH` واقعی
(headline/bio/city) تکمیل شد؛ یک checkout واقعی برای `PRO_MONTHLY` با
آداپتر mock انجام و دنبال شد تا یک سفارش `PAID` واقعی در Postgres ساخته
شود. اسکرین‌شات‌های واقعی تأیید کردند: صفحه‌ی پروفایل با کارت هدر/آواتار
حلقه‌دار/امتیاز/مهارت‌ها؛ `/pricing` با آیکون Crown هدر و آیکون
اختصاصی هر کارت (Crown/Bell)؛ `/payment/result?status=success` با
دایره‌ی سبز/primary و جزئیات سفارش واقعی (`refId` واقعی mock، مبلغ،
تاریخ پرداخت)؛ `/payment/result?status=failed` با دایره‌ی قرمز/destructive
متمایز — همه بدون هیچ خطای کنسول. داده‌ی تستی (۱ کاربر، ۱ سفارش) بعداً
مستقیم از Postgres واقعی پاک شد؛ شمارش پایه‌ی seed تأیید شد بدون تغییر
ماند: ۸ کاربر، ۰ سفارش، ۰ Entitlement، ۰ اشتراک (فاز ۹ عمداً بدون
سفارش/اشتراک seed‌شده کار می‌کند).
...
## فاز ۷ — AI (ویزارد ساخت درخواست با کمک هوش مصنوعی)

**چیستی:** کاربر به‌جای پرکردن فرم دستی درخواست (فاز ۵)، می‌تواند در یک
گفتگوی چندمرحله‌ای به فارسی توضیح آزاد بدهد؛ سرویس AI فیلدهای درخواست
(عنوان، توضیح، دسته‌بندی، حالت، شهر، مدت، بازه‌ی بودجه) را به‌تدریج
استخراج می‌کند و هر بار یک سؤال روشن‌کننده برای مهم‌ترین فیلد ناقص
می‌پرسد؛ وقتی چیزی کم نبود، پیش‌نویس آماده‌ی بررسی/انتشار است. مدل
`AiSession` از فاز ۱ در schema آماده بود (`messages`, `extractedDraft`,
`tokensUsed`, `provider`) ولی هیچ endpoint‌ای برایش نوشته نشده بود — این
فاز همان بن‌بست را می‌بندد.

### تصمیم‌های طراحی گرفته‌شده پیش از شروع کد

- **schema استخراج (`aiExtractedDraftSchema` در `packages/shared`) کاملاً
  nullable/optional است** — این یک پیش‌نویس در حال تکمیل است، نه یک
  payload آماده‌ی انتشار؛ اعتبارسنجی نهایی سخت‌گیرانه (`createRequestSchema`
  فاز ۵) وقتی کاربر پیش‌نمایش را در فرم موجود بررسی/ویرایش می‌کند دوباره
  اعمال می‌شود — یعنی فرم فاز ۵ عیناً reuse می‌شود، نه یک UI تأیید جدا.
- **`categoryId` را خودِ schema اعتبارسنجی نمی‌کند** — چون مدل هیچ
  دسترسی مستقیمی به DB ندارد. `AiService` بعد از هر پاسخ، `categoryId`
  را در برابر دسته‌های فعال واقعی چک می‌کند؛ یک شناسه‌ی ساختگی دقیقاً
  مثل یک خروجی ناقص‌الشکل باعث retry می‌شود.
- **retry+fallback دقیقاً طبق بند ۳ همین فایل:** یک بار retry با یک پیام
  اصلاحی، در صورت شکست دوم `needsManualForm:true` + پیام فارسی محترمانه‌ی
  دقیقاً همان متنی که در تصمیمات فنی نوشته شده بود («فعلاً نتوانستم
  درخواست را تحلیل کنم…»)، بدون پرتاب خطای HTTP — یک پاسخ `200` عادی که
  کلاینت را به فرم دستی هدایت می‌کند.
- **پیام اصلاحی retry به‌جای یک turn جدید، به انتهای همان پیام کاربر
  می‌چسبد، نه push یک `role:'user'` جداگانه.** کشف‌شده حین نوشتن تست: چون
  `messages` قبل از این تغییر همیشه با یک turn کاربر تمام می‌شد، اضافه‌کردن
  یک turn کاربر دیگر یعنی دو `user` پشت‌سرهم به سمت آداپتر واقعی Anthropic
  می‌رفت — Messages API آنتروپیک نقش‌های متناوب (user/assistant) انتظار
  دارد و به‌احتمال زیاد این را رد می‌کرد. رفع با `withCorrectiveSuffix()`:
  متن اصلاحی به `content` همان آخرین پیام کاربر append می‌شود، نه یک پیام
  جدید.
- **هر دور استخراج یک جفت کامل user/assistant در `AiSession.messages`
  ذخیره می‌شود، نه فقط پیام کاربر.** طراحی اولیه فقط پیام‌های کاربر را
  نگه می‌داشت (assistant هرگز persist نمی‌شد) — همین تست واحد
  (`continueSession` روی یک گفتگوی دومرحله‌ای) این نقص را نشان داد: بدون
  turn دستیار، هم تاریخچه‌ی نمایش‌داده‌شده به کاربر در UI ناقص می‌ماند
  (سؤال قبلی دستیار دیده نمی‌شود) و هم تماس بعدی به Anthropic باز همان
  مشکل تناوب نقش را داشت. رفع: `appendAssistantTurn()` بعد از هر استخراج،
  `clarifyingQuestion` (یا پیام «آماده است» یا پیام fallback) را به‌عنوان
  یک turn واقعی `assistant` ذخیره می‌کند.
- **آداپتر mock بدون هیچ فراخوانی شبکه، با regex/keyword ساده‌ی واقعی
  (نه placeholder بی‌معنی)** — طبق بند ۳ («پیش‌فرض لوکال: mock»): مبلغ
  (بازه‌ی «بین X تا Y میلیون تومان» یا «حدود X تومان»، با `tomanToRial`
  واقعی از `@vaqt/shared`)، مدت زمان (`\d+ دقیقه/ساعت`)، حالت
  (آنلاین/حضوری/هردو → HYBRID)، شهر (فهرست ثابت ۸ شهر بزرگ)، و دسته‌بندی
  (تطبیق substring نام دسته‌ی واقعی در متن، با `normalizeFa` هر دو طرف —
  همان تابع نرمال‌سازی فاز ۱، نه یک نسخه‌ی موازی). هدف: توسعه/تست محلی
  بدون نیاز به کلید API واقعی، نه شبیه‌سازی کیفیت مدل واقعی.
- **آداپتر Anthropic با `fetch` خام، بدون افزودن `@anthropic-ai/sdk` به
  وابستگی‌ها** — هم‌راستا با الگوی موجود `SmsIrAdapter` (که هم از
  `fetch` خام استفاده می‌کند، نه یک SDK). خروجی ابزار (tool use) عمداً
  `snake_case` است (طبیعی‌تر برای مدل)؛ نگاشت به شکل `camelCase`ی
  `AiExtractedDraft` فقط داخل همین یک فایل اتفاق می‌افتد، هیچ‌جای دیگر
  snake_case نمی‌بیند.
- **`category_id` در JSON Schema ابزار، به‌صورت پویا از دسته‌های فعال
  واقعی DB به یک `enum` تبدیل می‌شود** (نه یک رشته‌ی آزاد) — مدل مجبور
  است دقیقاً یکی از شناسه‌های واقعی را برگرداند یا `null`، نه شناسه‌ی
  ساختگی.
- **بدون گارد `RequireVerifiedPhone` روی endpointهای AI.** بند ۳ گیت
  واقعی برای ثبت پیشنهاد/انتشار درخواست را مشخص کرده (فازهای ۵ و ۶)؛
  ویزارد AI فقط به کاربر واردشده نیاز دارد (از `JwtAuthGuard` سراسری) —
  تأیید شماره فقط در لحظه‌ی انتشار واقعی درخواست (نه در مرحله‌ی گفت‌وگو)
  اعمال می‌شود، دقیقاً هم‌راستا با بند ۲۵ («`roleIntent` صرفاً UX است»).

### PR ۱ — بک‌اند AI (تکمیل‌شده)

`apps/api/src/ai/**` (کنترلر + سرویس + کانفیگ + دو DTO + دو آداپتر +
پرامپت فارسی جدا) + schema مشترک جدید (`packages/shared/src/schemas/ai.ts`).
اندپوینت‌ها RPC-style (`@Post`)، هم‌الگو با بقیه‌ی کنترلرهای این فاز:

- `POST /ai/sessions` — شروع نشست با اولین پیام
- `POST /ai/sessions/message` — ادامه‌ی همان نشست، پشت یک چک مالکیت
  دستی در خودِ سرویس (نه `RequireOwnership` decorator — چون نیازی به
  resolver جدا نبود: سرویس مستقیماً `session.userId !== userId` را چک
  می‌کند و برای هر دو حالت «نشست دیگری» و «نشست ناموجود» دقیقاً همان
  `404 NOT_FOUND` را برمی‌گرداند، بدون افشای وجود نشست دیگران)

**تست:** ۱۴ تست جدید — ۱۰ تست در `ai.service.spec.ts` روی Postgres واقعی
(شروع/ادامه‌ی نشست، persist شدن صحیح `AiSession`، retry روی شکل نامعتبر،
retry روی `categoryId` ساختگی، fallback بعد از دو شکست پیاپی، رد نشست
متعلق به کاربر دیگر، رد نشست ناموجود، و دو تست یکپارچگی با خودِ
`MockAiAdapter` واقعی نه یک test double) + ۴ تست در
`anthropic-ai.adapter.spec.ts` با `fetch` mock‌شده (بدون هیچ تماس شبکه‌ی
واقعی یا کلید API — شکل دقیق درخواست شامل `enum` پویای دسته‌ها، نگاشت
snake_case→camelCase پاسخ، خطای `401`، و پاسخ بدون `tool_use`). ۱۵ تست
schema در `packages/shared/src/schemas/ai.test.ts` (پوشش ۱۰۰٪).
`public-routes.spec.ts` به‌روز شد (`AiController` به لیست کنترلرها
اضافه شد؛ هیچ روت `@Public()` جدیدی نیست).

**اثبات زنده (E2E کامل، Postgres/Redis واقعی، آداپتر mock، سرور dev
واقعی که routeهای جدید را بدون ری‌استارت دستی شناسایی کرد):** دو کاربر
واقعی با OTP واقعی. کاربر اول یک پیام کامل فرستاد
(«یک ترجمه آنلاین در تهران نیاز دارم، حدود ۹۰ دقیقه طول می‌کشد و
بودجه‌ام بین ۱ تا ۲ میلیون تومان است.») → استخراج صحیح `mode:ONLINE`،
`city:تهران`، `durationMinutes:90`، `budgetMinRial:10000000`،
`budgetMaxRial:20000000`، ولی `categoryId:null` (چون seed هیچ دسته‌ی
«ترجمه» ندارد — رفتار درست، نه باگ) با `clarifyingQuestion` متناظر. یک
پیام دوم با نام دسته‌ی واقعی («این کار در دسته برنامه‌نویسی است») از
طریق `POST /ai/sessions/message` → `categoryId:"cat-programming"` صحیح
resolve شد، `missingFields:[]`، تاریخچه‌ی کامل چهار-turn‌ای
(user/assistant/user/assistant) در پاسخ و در Postgres یکسان بود. یک
پیام تک‌مرحله‌ای دیگر با نام دسته‌ی واقعی («یک کار برنامه‌نویسی آنلاین
نیاز دارم…») بدون هیچ پیام دومی مستقیم به پیش‌نویس کامل رسید. کاربر دوم
(نه صاحب نشست) هم روی نشست کاربر اول و هم روی یک `sessionId` ساختگی
دقیقاً همان `404 NOT_FOUND` گرفت. هر دو ردیف `ai_sessions` مستقیماً از
Postgres تأیید شدند (`provider:mock`, `tokensUsed:0` — درست، چون mock
هیچ توکنی مصرف نمی‌کند) و بعداً پاک شدند؛ شمارش پایه‌ی seed بدون تغییر
ماند: ۸ کاربر، ۱۵ درخواست، ۱۲ دسته، ۱۲ مهارت، ۲۰ پیشنهاد، ۲ گفتگو، ۰
نشست AI.

**آداپتر Anthropic واقعی هرگز زنده تست نشد** — این محیط به `ANTHROPIC_API_KEY`
دسترسی ندارد؛ فقط با `fetch` mock‌شده (بالا) و typecheck/lint تأیید شده،
نه با یک تماس واقعی به API آنتروپیک. صادقانه ثبت می‌شود تا بعداً با یک
کلید واقعی حتماً یک‌بار زنده تست شود، پیش از تکیه‌ی production روی آن.

`pnpm lint`/`typecheck`/`test` کامل مونوریپو سبز (۸/۸/۸ تسک، ۳۴۳ تست
apps/api، +۱۵ تست `packages/shared`).

### PR ۲ — ویزارد UI + پیش‌نمایش زنده (تکمیل‌شده)

`apps/web/src/app/requests/new/ai/page.tsx` (صفحه‌ی جدید، هم‌الگو با
`/requests/new`: نیازمند ورود، redirect به `/login` اگر نه) +
`apps/web/src/components/domain/ai-wizard.tsx` (کامپوننت اصلی) + یک
افزودنی کوچک به `request-form.tsx`: prop اختیاری جدید `initialValues`
(نوع اکسپورت‌شده‌ی `RequestFormPrefill`، فقط زیرمجموعه‌ای از فیلدهایی که
AI واقعاً استخراج می‌کند — `deadlineAt` و `preferredWindows` عمداً بیرون
ماندند چون استخراج مهلت/بازه‌ی زمانی از گفت‌وگوی آزاد خارج از scope این
فاز است) که وقتی داده شود، `useForm`ی داخلی به‌جای مقادیر خالی همیشگی،
با آن مقدار اولیه `reset` می‌شود.

**تصمیم‌های طراحی:**

- **مسیر مستقل `/requests/new/ai`، نه یک تب/toggle داخل همان صفحه‌ی
  دستی.** جریان دستی فاز ۵ دست‌نخورده باقی ماند (بدون هیچ ریسک رگرسیون
  روی چیزی که از قبل تست شده)؛ یک لینک متنی کوچک هر دو صفحه را به هم
  وصل می‌کند («امتحان با کمک هوش مصنوعی» / «پر کردن دستی فرم»).
- **«پیش‌نمایش زنده» یک کارت جداست که با هر پاسخ سرور به‌روز می‌شود** —
  دسته‌بندی با نام واقعی نمایش داده می‌شود (نه شناسه‌ی خام؛ کامپوننت
  خودش `GET /categories` را جدا فراخوانی می‌کند تا نگاشت id→name را
  داشته باشد)، حالت همکاری از `fa.requestMode` (رجیستری موجود فاز ۵،
  نه یک نگاشت موازی)، و بازه‌ی بودجه با `formatToman` واقعی از
  `@vaqt/shared`.
- **دکمه‌ی «بررسی و انتشار» فقط وقتی `missingFields` خالی باشد ظاهر
  می‌شود؛ یک دکمه‌ی «پرکردن دستی فرم» همیشه در دسترس است** — کاربر
  می‌تواند هر لحظه (حتی وسط گفت‌وگو، با پیش‌نویس ناقص) به فرم دستی
  فرار کند؛ فیلدهای هنوز `null` به‌سادگی در فرم خالی می‌مانند.
- **وقتی `needsManualForm:true` برگردد، بلافاصله (بدون هیچ دکمه‌ای)
  به فرم دستی سوییچ می‌شود** — همراه با نمایش پیام fallback بالای فرم؛
  دقیقاً رفتار مستندشده در بند ۳ («fallback به فرم دستی»).
- **بدون هیچ به‌روزرسانی خودکار/polling در چت** — هر پیام یک
  request/response ساده است، بدون نیازی به WebSocket (بر خلاف فاز ۸)
  چون این یک گفتگوی تک‌کاربره‌ی همزمان است، نه یک چت دوطرفه‌ی بی‌درنگ.

**اثبات زنده (Playwright، جریان کامل واقعی، Postgres/Redis واقعی، OTP
واقعی، بدون هیچ mock به‌جز `SMS_PROVIDER=mock`):** یک کاربر واقعی وارد
شد، به `/requests/new/ai` رفت، یک پیام کامل فرستاد («یک کار
برنامه‌نویسی آنلاین در تهران نیاز دارم، حدود ۶۰ دقیقه طول می‌کشد و
بودجه‌ام بین ۱ تا ۲ میلیون تومان است.») → پیش‌نمایش زنده بلافاصله
`دسته: برنامه‌نویسی`، `شهر: تهران`، `مدت: ۶۰`، `بودجه: ۱٬۰۰۰٬۰۰۰ –
۲٬۰۰۰٬۰۰۰ تومان` را نشان داد و دکمه‌ی «بررسی و انتشار» ظاهر شد. کلیک
روی آن → فرم `RequestForm` با همه‌ی این مقادیر واقعاً پرشده باز شد
(اسکرین‌شات: عنوان/توضیحات/دسته/نوع همکاری/شهر/مدت/بودجه همگی صحیح،
فقط «مهلت» خالی — دقیقاً طبق طراحی). کاربر مهلت را دستی پر کرد، «ثبت
پیش‌نویس» → پیش‌نمایش انتشار → «انتشار» → ریدایرکت به `/requests`.
تأیید مستقیم از Postgres: ردیف واقعی با `status:PUBLISHED` و همان
عنوان/دسته/شهر/مدت/بودجه‌ی استخراج‌شده توسط AI.

**باگ محیطی کشف‌شده حین این QA (نه در کد این PR):** سرور dev طولانی‌مدت
`next dev` (از فاز ۸ باز مانده بود) دوباره همان cache وبپک خراب‌شده‌ی
مستندشده در فاز ۸/PR۳ را نشان داد (`__webpack_modules__[moduleId] is
not a function`، خطای React Client Manifest). رفع با همان روش قبلی
(`rm -rf apps/web/.next` + ری‌استارت)، بدون تغییر کد — یک الگوی تکرارشونده
که به بدهی فنی اضافه نشد چون ریشه‌اش شناخته‌شده و خودمحدود است (فقط
سرور dev طولانی‌مدت را می‌گیرد، نه build واقعی production).

داده‌ی تستی (۱ کاربر، ۱ نشست AI، ۱ درخواست) از Postgres واقعی پاک شد؛
شمارش پایه‌ی seed تأیید شد بدون تغییر ماند: ۸ کاربر، ۱۵ درخواست، ۱۲
دسته، ۱۲ مهارت، ۲۰ پیشنهاد، ۲ گفتگو، ۰ نشست AI.

`pnpm lint`/`typecheck`/`build`/`test` کامل مونوریپو سبز (بودجه‌ی باندل
`apps/web` سبز؛ صفحه‌ی جدید `/requests/new/ai` استاتیک، ۱٫۹ کیلوبایت).

**فاز ۷ اکنون کاملاً تکمیل است** — بک‌اند (نشست/استخراج/retry-fallback)
...
### PR ۲ — UI پرداخت (صفحه‌ی نتیجه + دکمه‌های خرید، تکمیل‌شده)

آخرین بخش فاز ۹. `GET /payments/products` جدید (کاتالوگ عمومی، هر پنج
محصول seed‌شده، ارزان‌ترین اول، بدون افشای `id` داخلی) + `RequestDetail`
سه فیلد `isUrgent`/`isFeatured`/`bumpedAt` گرفت تا فرانت‌اند بدون
endpoint تازه، اثر خرید را منعکس کند.

- `PurchaseButton` — شروع checkout و ریدایرکت کامل مرورگر به `redirectUrl`
  (بدون هیچ حالت «موفق» داخل خودِ کامپوننت، چون کل مرورگر جابه‌جا می‌شود)
- `RequestUpgrades` — پنل سه محصول درخواست‌محور (`URGENT_BADGE`/`BUMP`/
  `FEATURE`) روی صفحه‌ی جزئیات درخواست، فقط برای مالک و فقط وقتی
  `PUBLISHED`؛ دکمه‌ی محصولی که از قبل خریداری شده غیرفعال می‌شود
- `/pricing` — دو محصول حساب‌محور (`PRO_MONTHLY`/`TARGETED_NOTIFY`)
- `/payment/result` — رندر نتیجه‌ی callback (`status=success|failed|
not_found`) + جزئیات کامل سفارش از `GET /payments/order?id=`

**اثبات زنده (E2E کامل روی سرور dev واقعی، Postgres/Redis واقعی، آداپتر
mock):** OTP واقعی → ساخت و انتشار درخواست → `GET /payments/products`
(پنج محصول، بدون `id`) → `POST /payments/checkout` با `URGENT_BADGE` روی
همان درخواست → دنبال‌کردن `redirectUrl` (که برای mock مستقیم به همان
callback با `Status=OK` اشاره می‌کند) → `302` موفق → `GET /requests/:id`
بلافاصله `isUrgent:true` نشان داد → `GET /payments/order?id=` سفارش
`PAID` با `refId`/`paidAt` واقعی برگرداند، دقیقاً هم‌شکل با چیزی که صفحه‌ی
نتیجه مصرف می‌کند. داده‌ی تستی این اجرا به‌علاوه‌ی داده‌ی باقی‌مانده از یک
بازبینی قبلی (۵ کاربر/۲ درخواست/۳ سفارش/۳ Entitlement که هرگز پاک نشده
بود) هر دو از Postgres واقعی حذف شدند؛ شمارش پایه‌ی seed تأیید شد بدون
تغییر ماند: ۸ کاربر، ۱۵ درخواست، ۱۲ دسته، ۱۲ مهارت، ۲۰ پیشنهاد، ۲ گفتگو،
۰ سفارش، ۰ Entitlement.

`pnpm lint`/`typecheck`/`test`/`build` کامل مونوریپو سبز (۸/۸/۸/۵ تسک،
۳۶۴ تست apps/api بدون تغییر). **نکته‌ی محیطی حین این بازبینی:** اجرای
`pnpm build` با `NODE_ENV=development` صادرشده در شل باعث شکست ظاهراً
بی‌ربط apps/web شد (`<Html> should not be imported outside of pages/
_document`, در واقع یک خطای گمراه‌کننده‌ی Next.js وقتی build با
`NODE_ENV` غیر‌استاندارد اجرا شود، نه یک باگ در کد این PR) — با build
مستقیم (`NODE_ENV` تنظیم‌نشده) رفع شد؛ تست/typecheck همچنان باید با
`NODE_ENV=development` اجرا شوند (طبق env-validation)، فقط build نباید.

**فاز ۹ اکنون کاملاً تکمیل است** — بک‌اند و UI هر دو merge شده‌اند
(PR ۳۰ بک‌اند+catalog، PR ۳۱ UI). تنها محدودیت باقی‌مانده (صادقانه
ثبت‌شده در بک‌اند): آداپتر واقعی Zarinpal هرگز با merchant id واقعی زنده
تست نشد.

---

## فاز ۱۰ — تکمیل تجربه (نظرات/امتیازدهی + پروفایل عمومی + PWA + SEO)

**چیستی:** طبق `PROJECT_SPEC.md` بند ۶ و جدول فازها، این فاز چهار بخش عمدتاً
مستقل را می‌بندد: (۱) نظرات و امتیازدهی بعد از یک همکاری، (۲) صفحه‌ی پروفایل
عمومی که آمار و نظرات را نشان می‌دهد، (۳) PWA پایه (manifest، آیکون‌ها،
theme-color)، (۴) SEO (metadata فارسی، sitemap، robots، JSON-LD صفحه‌ی
درخواست، OG Image داینامیک). مدل `Review` از فاز ۱ در schema آماده بود
(`conversationId` یکتا به‌ازای هر reviewer، `reviewerId`, `revieweeId`,
`rating` ۱ تا ۵، `comment`, `isVisible`) ولی هیچ endpoint‌ای نداشت؛
`User.ratingAvg`/`ratingCount` هم از فاز ۳ فقط با پیش‌فرض ۰ اضافه شده بودند
و هرگز محاسبه نمی‌شدند (بند ۳۰ همین فایل، به‌صراحت به «فاز بازبینی» موکول
شده بود).

### تصمیم‌های طراحی گرفته‌شده پیش از شروع کد

- **مالکیت نظر بدون اعتماد به کلاینت:** `revieweeId` هرگز از بدنه‌ی درخواست
  گرفته نمی‌شود — از خودِ گفتگو resolve می‌شود («طرف مقابل reviewer»). چون
  `seekerId` و `providerId` یک گفتگو هرگز یک نفر نیستند (`OWN_REQUEST_OFFER_FORBIDDEN`
  از فاز ۶ این را تضمین می‌کند)، self-review ساختاری غیرممکن است — نیازی
  به یک چک صریح جدا نبود.
- **بدون شرط «گفتگو باید بسته/تمام‌شده باشد».** هیچ ویژگی «علامت‌گذاری
  پایان همکاری» در این پروژه وجود ندارد؛ تحمیل چنین شرطی یک قابلیت جدید و
  کامل جداگانه می‌طلبید که خارج از scope همین فاز است. تصمیم: نظر از همان
  لحظه‌ای که گفتگو ساخته می‌شود (یعنی از لحظه‌ی انتخاب پیشنهاد) قابل ثبت
  است، صرف‌نظر از وضعیت `OPEN`/`ARCHIVED` گفتگو.
- **یکتایی سخت (`@@unique([conversationId, reviewerId])`) به‌جای reactivate.**
  برخلاف الگوی resubmit پیشنهادها (بند ۱۴)، اینجا هیچ رفتار «ویرایش نظر
  بعد از ثبت» تعریف نشده — نظر بعد از ثبت غیرقابل‌تغییر است؛ تلاش دوم با
  `REVIEW_ALREADY_EXISTS` (۴۰۹) رد می‌شود. ویرایش/حذف نظر عمداً از این
  برش بیرون ماند (نیاز به تصمیم محصولی جدا درباره‌ی پنجره‌ی زمانی ویرایش).
- **`ratingAvg` با aggregate واقعی محاسبه می‌شود، نه افزایش تدریجی.** برخلاف
  الگوی «عملیات اتمیک SQL» برای شمارنده‌های پرتراکنش (بند ۱۰ قواعد
  دیتابیس)، این‌جا هر بار یک `prisma.review.aggregate({where:{revieweeId,
isVisible:true}})` واقعی زده می‌شود تا میانگین را از صفر بازمحاسبه کند —
  چون تعداد نظرات هر کاربر کوچک است (ده‌ها، نه هزاران) و این کار از انباشت
  خطای اعشاری در یک میانگین متحرک تدریجی جلوگیری می‌کند؛ کل عملیات (insert
  نظر + محاسبه‌ی aggregate + بروزرسانی User) در یک `prisma.$transaction`
  اتمیک است (طبق قاعده‌ی «چند-جدولی = تراکنش»).
- **`POST /reviews/status`** (endpoint جدید، کوچک): فرانت‌اند نباید حدس
  بزند که آیا کاربر قبلاً نظر داده یا نه با امتحان ارسال و گرفتن خطا —
  دقیقاً هم‌الگو با `myOfferId`/`myOfferStatus` که `RequestsService.getById`
  در فاز ۶ برای همین منظور اضافه کرده بود.
- **`POST /reviews/list` نیازمند ورود است، نه `@Public()`** — طبق همان
  قاعده‌ی موجود برای `GET /users/:id` (بند ۲۷: «پروفایل فقط برای کاربران
  واردشده قابل مشاهده است»)، برای یکدستی؛ صفحه‌ی پروفایل عمومی (بخش UI این
  فاز، هنوز پیاده نشده) هم به همین دلیل به ورود نیاز خواهد داشت.
- **صفحه‌بندی نظرات cursor-based (`createdAt DESC, id DESC`)**، دقیقاً
  هم‌الگو با `listMessages` (فاز ۸) — نه یک آرایه‌ی کامل بدون صفحه‌بندی
  مثل `offers.listForRequest` — چون نظرات یک کاربر می‌تواند در طول زمان
  زیاد شود (صفحه‌ی پروفایل عمومی، برخلاف لیست پیشنهادهای یک درخواست).

### PR ۱ — بک‌اند نظرات/امتیازدهی (تکمیل‌شده)

`apps/api/src/reviews/**` (کنترلر + سرویس + ۳ DTO) + schema مشترک جدید
(`packages/shared/src/schemas/review.ts`) + کد خطای جدید
`REVIEW_ALREADY_EXISTS`. اندپوینت‌ها RPC-style (`@Post`)، هم‌الگو با
`ConversationsController`:

- `POST /reviews` — ثبت نظر، پشت `RequireOwnership` (seeker یا provider
  همان گفتگو؛ resolver محلی، هم‌شکل `resolveParticipantsFromBody` در
  `conversations.controller.ts` ولی چون آنجا export نشده بود، نسخه‌ی محلی
  خودش نوشته شد — نه یک وابستگی cross-module غیرضروری برای یک تابع کوچک)
- `POST /reviews/status` — وضعیت «آیا من نظر داده‌ام»، پشت همان `RequireOwnership`
- `POST /reviews/list` — لیست نظرات دریافتی یک کاربر، صفحه‌بندی‌شده، بدون
  گارد مالکیت (هرکسی که وارد شده می‌تواند نظرات هرکسی را ببیند — نظرات
  عمومی هستند، برخلاف پیام‌های گفتگو)

**تست:** ۹ تست جدید در `reviews.service.spec.ts` روی Postgres واقعی (بدون
mock؛ گفتگوها از طریق جریان واقعی `OffersService.submit()`→`select()`
ساخته می‌شوند، دقیقاً مثل `conversations.service.spec.ts`)، پوشش‌دهنده‌ی:
ثبت نظر از سمت seeker و به‌روزرسانی `ratingAvg`/`ratingCount` provider،
derive صحیح revieweeId وقتی provider نظر می‌دهد، میانگین‌گیری صحیح روی
چند گفتگوی مختلف برای یک reviewee، رد نظر دوم روی همان گفتگو، رد گفتگوی
ناموجود، لیست newest-first با اطلاعات reviewer، صفحه‌بندی با cursor
(۳ نظر، limit=2)، حذف نظرات `isVisible:false` از لیست، و `myReviewStatus`
قبل/بعد از ثبت. ۱۴ تست schema جدید در `packages/shared/src/schemas/review.test.ts`
(پوشش ۱۰۰٪). `public-routes.spec.ts` به‌روز شد (`ReviewsController` به
لیست کنترلرها اضافه شد؛ هیچ روت `@Public()` جدیدی نیست).

**اثبات زنده (E2E کامل، Postgres/Redis واقعی، آداپتر mock، سه کاربر واقعی
با OTP واقعی):** seeker (A) درخواست ساخت و منتشر کرد؛ provider (B) با
پروفایل تکمیل‌شده (bio+مهارت) پیشنهاد داد؛ A انتخاب کرد → گفتگوی واقعی.
سپس: `POST /reviews/status` قبل از ثبت → `{"reviewed":false}`؛
`POST /reviews` با `rating:5` + کامنت → ردیف واقعی با `reviewer` resolve‌شده؛
`POST /reviews/status` بعد از ثبت → `{"reviewed":true}`؛ تلاش دوم →
`409 REVIEW_ALREADY_EXISTS`؛ کاربر سوم C (نه seeker نه provider) روی
`POST /reviews/status` همان گفتگو → `403 FORBIDDEN`. تأیید مستقیم از
Postgres: `users.ratingAvg=5, ratingCount=1` برای B؛ `POST /reviews/list`
برای B همان نظر را برگرداند؛ `GET /users/:id` برای B هم `ratingAvg:5,
ratingCount:1` را در پاسخ عمومی نشان داد — یعنی زنجیره‌ی کامل از ثبت نظر
تا نمایش در پروفایل عمومی، سرتاسر روی سرور واقعی تأیید شد. تمام داده‌ی
تستی (۳ کاربر، ۱ درخواست، ۱ پیشنهاد، ۱ گفتگو، ۱ نظر) بعداً از Postgres
واقعی پاک شد؛ شمارش پایه‌ی seed تأیید شد بدون تغییر ماند: ۸ کاربر، ۱۵
درخواست، ۱۲ دسته، ۱۲ مهارت، ۲۰ پیشنهاد، ۲ گفتگو، ۲ نظر (از seed خودِ فاز ۱،
نه از این تست).

`pnpm lint`/`typecheck`/`test` کامل مونوریپو سبز (۸/۸/۸ تسک، ۳۹۷ تست
apps/api، +۹ نسبت به قبل).

**باقی‌مانده برای فاز ۱۰ (پیش از PR ۲):** UI نظرات (فرم ثبت نظر در صفحه‌ی چت، نمایش
نظرات در پروفایل عمومی)، صفحه‌ی `/profile` عمومی با آمار «N همکاری موفق»،
PWA (manifest، آیکون‌ها، theme-color)، SEO (metadata فارسی، sitemap،
robots، JSON-LD صفحه‌ی درخواست، OG Image داینامیک برای اشتراک‌گذاری
لینکدین).

### PR ۲ — UI نظرات + پروفایل عمومی (تکمیل‌شده)

`apps/web/src/components/domain/star-rating.tsx` (کامپوننت ستاره‌ی
قابل‌استفاده‌ی دوباره — حالت `readOnly` برای نمایش، حالت تعاملی برای
انتخاب امتیاز با `onChange`) + `review-panel.tsx` (روی صفحه‌ی گفتگو،
هم‌الگو با `offers-panel.tsx`: وضعیت با `POST /reviews/status` گرفته
می‌شود، اگر نظر داده نشده فرم امتیاز+نظر نشان داده می‌شود، اگر داده شده
همان امتیاز/نظر ثبت‌شده به‌صورت `readOnly` نمایش داده می‌شود) +
`apps/web/src/app/users/[id]/page.tsx` (پروفایل عمومی: آواتار، headline،
شهر، امتیاز، مهارت‌ها، بیوگرافی، و لیست نظرات دریافتی با صفحه‌بندی
cursor-محور از `POST /reviews/list`).

**دو تغییر کوچک بک‌اند لازم برای این UI:**

- `myReviewStatus()` قبلاً فقط `{reviewed: boolean}` برمی‌گرداند؛ برای
  اینکه پنل نظر بتواند بعد از رفرش صفحه همان امتیاز/نظر ثبت‌شده را نشان
  دهد (نه فقط یک پیام عمومی «نظر داده‌اید»)، اکنون `{reviewed, rating,
comment}` برمی‌گرداند.
- `ConversationSummary`/`toSummary()` یک فیلد جدید `counterpartId` گرفت
  (شناسه‌ی کاربر طرف مقابل) — بدون آن، لینک کردن نام طرف مقابل در هدر
  گفتگو به پروفایل عمومی‌اش ممکن نبود.

نام ارائه‌دهنده در کارت‌های پیشنهاد (`offers-panel.tsx`، از `providerId`
که قبلاً هم در `OfferSummary` وجود داشت) و نام طرف مقابل در هدر صفحه‌ی
گفتگو، هر دو اکنون به `/users/[id]` لینک می‌شوند.

**اثبات زنده (E2E کامل، Postgres/Redis واقعی، سرور dev واقعی، دو کاربر
واقعی با OTP واقعی):** seeker درخواست ساخت و منتشر کرد؛ provider (با
پروفایل تکمیل‌شده) پیشنهاد داد؛ seeker انتخاب کرد → گفتگوی واقعی.
`POST /conversations/mine` → `GET /conversations/:id` تأیید کرد
`counterpartId` دقیقاً برابر شناسه‌ی provider است. `POST /reviews/status`
قبل از ثبت → `{reviewed:false, rating:null, comment:null}`؛
`POST /reviews` با امتیاز ۵ و نظر → موفق؛ `POST /reviews/status` بعد از
ثبت → `{reviewed:true, rating:5, comment:"..."}`. `GET /users/:id` روی
provider بلافاصله `ratingAvg:5, ratingCount:1` و مهارت‌های واقعی نشان
داد — یعنی زنجیره‌ی کامل از ثبت نظر تا محاسبه‌ی aggregate تا نمایش در
پروفایل عمومی، سرتاسر روی سرور واقعی تأیید شد. `POST /reviews/list`
برای provider همان نظر را با اطلاعات reviewer برگرداند.

**دو باگ اسکریپت QA (نه در کد محصول) کشف و رفع شدند حین این اثبات:**
(۱) میدل‌ور `origin-check` بدون هدر `Origin: http://localhost:3000` روی
هر `POST` جدید هم ۴۰۳ می‌دهد — دقیقاً همان رفتار مستندشده در فاز ۵/PR۲،
هر فراخوانی curl این هدر را نیاز داشت؛ (۲) استخراج OTP از لاگ سرور با
grep روی رنگ‌های ANSI ناپایدار بود (شماره‌ی ماسک‌شده در لاگ، نه شماره‌ی
کامل، و کد OTP بلافاصله بعد از یک کد رنگ ANSI می‌آمد) — رفع با
`sed -E 's/\x1b\[[0-9;]*m//g'` برای حذف کدهای رنگ قبل از استخراج رقم.

پاک‌سازی: کل داده‌ی تستی این بازبینی (۱۰ کاربر تستی انباشته از چند اجرای
اسکریپت debug، ۲ درخواست، ۲ پیشنهاد، ۱ گفتگو، ۱ نظر) با
`cleanup:qa --execute --older-than-minutes=0` پاک شد (۱۹ کلید Redis هم
حذف شدند)؛ شمارش نهایی seed با کوئری مستقیم تأیید شد بدون تغییر ماند:
۸ کاربر، ۱۵ درخواست، ۱۲ دسته، ۱۲ مهارت، ۲۰ پیشنهاد، ۲ گفتگو، ۲ نظر.

`pnpm lint`/`typecheck`/`build`/`test` کامل مونوریپو سبز (۸/۸/۸/۵ تسک،
۳۹۷ تست apps/api بدون تغییر؛ بودجه‌ی باندل `apps/web` سبز، مسیر جدید
`/users/[id]` به‌درستی `ƒ (Dynamic)`). PR #35، merge شده روی `main` پس از
سبز شدن CI.

### PR ۳ — PWA (manifest، آیکون‌ها، theme-color، تکمیل‌شده)

`apps/web/src/app/manifest.ts` (نام/توضیح فارسی، `dir:'rtl'`، `lang:'fa'`،
`display:'standalone'`، رنگ برند) + `icon.tsx`/`apple-icon.tsx` (از طریق
قراردادهای فایل ویژه‌ی Next — فاویکون و آیکون اپل) + دو روت دستی
`icons/icon-192`/`icons/icon-512` (چون قراردادهای `icon.tsx` فقط یک تصویر
به‌اندازه‌ی فاویکون تولید می‌کنند، ولی prompt نصب PWA اندروید/کروم به
ورودی‌های ۱۹۲ و ۵۱۲ در آرایه‌ی `icons` مانیفست نیاز دارد) + انتقال
`themeColor`/`colorScheme` از `metadata` به یک export جدای `viewport`
(از Next 14 به بعد ماندن آن‌ها داخل `metadata` بی‌صدا no-op می‌شود).

**اثبات زنده (نه فقط build):** با `next start` واقعی، `curl` تأیید کرد
`manifest.webmanifest` با `content-type: application/manifest+json` و
فیلدهای فارسی/RTL درست سرو می‌شود؛ هر چهار روت آیکون (`icon`,
`apple-icon`, `icons/icon-192`, `icons/icon-512`) واقعاً `image/png`
برمی‌گردانند؛ و `<head>` رندرشده به‌طور خودکار شامل
`<link rel="manifest">`, `<meta name="theme-color">`,
`<link rel="icon">`, `<link rel="apple-touch-icon">` است.

`pnpm typecheck`/`build` سبز. PR #37، merge شده روی `main` پس از سبز
شدن CI.

### PR ۴ — SEO صفحه‌ی درخواست (metadata، JSON-LD، sitemap، robots، OG Image، تکمیل‌شده)

**تصمیم طراحی کلیدی:** `GET /requests/:id` (فاز ۶) نیازمند ورود است —
crawlerها و بات‌های unfurl لینک (لینکدین و غیره) هرگز کوکی session
ندارند. به‌جای باز کردن آن اندپوینت یا افشای فیلدهای حساس (owner id،
بودجه، myOffer)، `POST /requests/list` که از قبل `@Public()` است یک
فیلتر اختیاری `id` گرفت — دقیقاً همان شکل بدون‌بودجه‌ی PUBLISHED-only
بقیه‌ی همان اندپوینت را برمی‌گرداند، بدون سطح جدیدی از افشا. تست‌های
جدید در `requests.service.spec.ts`: فیلتر به یک آیتم، رد DRAFT (بدون
نشتی وجود از طریق اندپوینت عمومی)، رد id ناموجود.

**فرانت‌اند:** `requests/[id]/page.tsx` به یک Server Component تبدیل شد
(`generateMetadata` + JSON-LD نوع `Demand` از schema.org — دقیقاً نوع
درستی برای یک «آگهی خواستار» به‌جای یک آگهی عرضه‌کننده) که کامپوننت
کلاینت قبلی (بدون تغییر، فقط rename به `request-detail-client.tsx`) را
رندر می‌کند. `robots.ts` صفحات نیازمند ورود بدون ارزش برای بات
(`/conversations/*`, `/users/*`, `/requests/new*`, `/payment/result`,
`/dev/ui`) را disallow می‌کند. `sitemap.ts` با همان الگوی
batch-processing این پروژه (سقف `MAX_PAGES`) تمام درخواست‌های منتشرشده
را از طریق همان اندپوینت عمومی صفحه‌بندی می‌کند.

**دو باگ واقعی کشف‌شده حین ساخت (نه صرفاً کدنویسی طبق مستندات):**

1. **`origin-check.middleware` هر `POST` را mutating می‌داند** — دقیقاً
   همان تله‌ای که فاز ۵/PR۲ صفحه‌ی فهرست را شکست: یک fetch سمت سرور
   (بدون مرورگر) به `POST /requests/list` بدون هدر `Origin` با `403`
   رد می‌شود. رفع: متغیر محیطی جدید `WEB_ORIGIN` روی `apps/web` (باید
   دقیقاً با `WEB_ORIGIN` سمت `apps/api` یکی باشد) که به‌صراحت به‌عنوان
   هدر `Origin` پاس داده می‌شود — یک فراخوانی مشروع همان سیستم، نه
   جعل، چون مقدار همان origin پیکربندی‌شده‌ی خودمان است.
2. **رندرکننده‌ی `next/og` (Satori/resvg) فونت WOFF2 پروژه را اصلاً
   نمی‌فهمد** — خطای `Unsupported OpenType signature wOF2`؛ رفع با
   دریافت یک TTF از همان ریلیز رسمی `rastikerdar/vazirmatn` که بند ۲
   همین فایل به آن اشاره می‌کند (`src/assets/fonts/Vazirmatn-Bold.ttf`).
   **جداگانه، با اجرای زنده کشف شد که همین رندرکننده نیم‌فاصله (ZWNJ،
   U+200C) را هم اشتباه مدیریت می‌کند** — کلمه‌ی مرکب «توسعه‌دهنده‌ی»
   به «ی‌دهندهتوسعه» درهم‌ریخته می‌شد، چون هر بخش جداشده با ZWNJ به‌عنوان
   یک run دوسویه‌ی (bidi) مستقل و به‌ترتیب معکوس رندر می‌شد — با اینکه
   متن ساده‌ی چندکلمه‌ای بدون ZWNJ و چسبیدن حروف داخل یک کلمه هر دو درست
   کار می‌کردند. چون نیم‌فاصله در فارسی واقعی رایج است (تابع خودِ پروژه
   `normalizeFa()` آن را یک کاراکتر درجه‌یک می‌داند)، هر رشته پیش از
   رندر با `stripZwnj()` به فاصله‌ی معمولی تبدیل می‌شود — افت جزئی
   تایپوگرافی در برابر یک عنوان کاملاً درهم‌ریخته.

**اثبات زنده (E2E کامل روی سرور dev واقعی، Postgres/Redis واقعی، یک
درخواست واقعی منتشرشده):** `curl` روی `<head>` رندرشده تأیید کرد
`<title>`, `<meta name="description">`, `og:title/description/image`,
`twitter:*`، و بلوک JSON-LD همگی از داده‌ی واقعی درخواست ساخته شده‌اند؛
`og:image` به‌صورت خودکار (بدون نیاز به ذکر دستی در `generateMetadata`)
به روت `opengraph-image` وصل شد. تصویر واقعی OG (`image/png` واقعی)
دانلود و بازرسی بصری شد — هم قبل از رفع باگ ZWNJ (متن درهم‌ریخته) و هم
بعد از رفع (ترتیب کلمات و اتصال حروف کاملاً درست، «نیاز به توسعه دهنده
ی فرانت اند برای رفع باگ» خوانا و به‌ترتیب صحیح). داده‌ی تستی با
`cleanup:qa --execute --older-than-minutes=0` پاک شد؛ شمارش seed تأیید
شد بدون تغییر ماند.

`pnpm lint`/`typecheck`/`test` کامل مونوریپو سبز (۵۰/۵۰ suite، ۴۰۰ تست
apps/api، +۳ نسبت به قبل؛ `request.ts` در `packages/shared` همچنان
۱۰۰٪ پوشش). PR #38، merge شده روی `main` پس از سبز شدن CI.

**فاز ۱۰ اکنون کاملاً تکمیل است** — نظرات/امتیازدهی، پروفایل عمومی،
PWA، و SEO صفحه‌ی درخواست، همگی merge شده‌اند.

**تصمیم محصولی عمداً باز گذاشته‌شده (کشف‌شده حین این بازبینی، نه یک باگ):**
`apps/web/src/app/page.tsx` (صفحه‌ی اصلی واقعی سایت) هنوز همان کارت
نمایشی کامپوننت‌های فاز ۴ است — متن «فاز ۴ — سیستم طراحی» را به بازدیدکننده‌ی
واقعی نشان می‌دهد و دکمه‌ی «شروع کنید» هیچ `href`/handler‌ای ندارد. چون
metadata سطح سایت (`layout.tsx`) از قبل فارسی و درست است، این محتوا
هیچ‌جا SEO یا PWA را نمی‌شکند — ولی به این معناست که صفحه‌ی اصلی هنوز
...
## فاز ۱۱ — کیفیت و تحویل (تکمیل‌شده)
...
production برای هر دو اپ، (۳) بازبینی امنیتی. هر سه تکمیل و merge شدند.
...
### PR ۳ — بازبینی امنیتی (تکمیل‌شده)

بازبینی مستقیم کد + `pnpm audit` روی استک واقعی، نه فقط چک‌لیست نظری. سه یافته‌ی
واقعی پیدا و رفع شد؛ دو مورد باقی‌مانده (هر دو نیازمند یک ارتقای نسخه‌ی اصلی
شکننده) عمداً به بدهی فنی موکول شدند.

**یافته‌های رفع‌شده:**

1. **بدون هدر امنیتی HTTP.** `helmet` هرگز نصب/فعال نشده بود — بدون
   `X-Frame-Options`، `X-Content-Type-Options`، `Strict-Transport-Security`
   و بقیه. اضافه شد در `main.ts` با `contentSecurityPolicy: false` (این یک
   API خالص JSON/WebSocket است، بدون HTML سمت‌سرور به‌جز Swagger UI که خودش
   dev-only شد — پس CSP سخت‌گیرانه‌ی پیش‌فرض helmet نیازی ندارد و فقط
   Swagger را می‌شکند) و `crossOriginResourcePolicy: {policy: 'cross-origin'}`
   (چون `apps/web` روی origin/پورت دیگری تصاویر آواتار را مستقیم از
   `/uploads` همین سرور می‌خواند؛ پیش‌فرض `same-origin` helmet آن را می‌بست).
   اثبات زنده: `curl -i` روی `/api/v1/health` هر ۱۱ هدر helmet را نشان داد،
   بدون شکستن CORS موجود.
2. **مستندات Swagger بدون قید محیط در `/docs` عمومی expose می‌شد.** کل
   schema کامل API (هر route، شکل هر DTO، الزامات auth) بدون هیچ احراز
   هویتی در production هم در دسترس بود — افشای اطلاعات غیرضروری. رفع:
   `SwaggerModule.setup` فقط وقتی `NODE_ENV !== 'production'` اجرا می‌شود.
   اثبات زنده: بوت واقعی سرور با `NODE_ENV=production` (پورت جدا، بدون
   دست‌زدن به سرور dev اصلی) → `GET /docs` → `404`؛ `GET /api/v1/health`
   همزمان → `200` (بقیه‌ی API دست‌نخورده).
3. **۳۳ آسیب‌پذیری واقعی در `pnpm audit`، اکثراً در مسیر production
   (نه فقط ابزار build).** مهم‌ترین‌ها: **multer** (وابستگی مستقیم آپلود
   آواتار) با ۵ CVE با شدت high/moderate از نوع DoS (`incomplete cleanup`،
   `resource exhaustion`، `uncontrolled recursion`، `deeply nested field
names`)، و **sharp** (پردازش/EXIF-strip آواتار) با یک CVE با شدت high
   (آسیب‌پذیری‌های به‌ارث‌رسیده از `libvips`). بررسی شد که
   `@nestjs/platform-express@10.4.22` نصب‌شده از قبل خودش به multer 2.x
   resolve می‌کند (نه 1.x که در `package.json` نوشته شده بود) — یعنی ارتقای
   multor به `^2.2.0` (نسخه‌ی کاملاً رفع‌شده) با نسخه‌ی فعلی NestJS ۱۰ سازگار
   بود، بدون نیاز به ارتقای خودِ NestJS. رفع: `sharp` به `^0.35.4` و `multer`
   به `^2.2.0` در `apps/api/package.json`، به‌علاوه‌ی `pnpm.overrides` در ریشه
   برای شش وابستگی تراکنشی دیگر (`js-yaml`, `qs`, `lodash`, `postcss`,
   `file-type`, `deepmerge-ts`, `body-parser`, `fast-uri`) که همگی
   bump-های patch/minor بدون تغییر API بودند. نتیجه: **۳۳ → ۲** آسیب‌پذیری
   (دو مورد باقی‌مانده پایین مستند شدند). اثبات زنده: آپلود آواتار واقعی
   (با EXIF ساختگی) از طریق `POST /users/me/avatar` روی سرور dev واقعی با
   multer/sharp جدید انجام شد — پاسخ موفق، فایل اصلی دقیقاً ۴۰۰×۴۰۰ و
   thumbnail دقیقاً ۹۶×۹۶، و `exif` روی هر دو فایل خروجی `false` (حذف کامل)،
   دقیقاً هم‌رفتار نسخه‌های قدیمی.

**بدون تغییر (بررسی و تأیید شد که از قبل درست بودند):** کوکی‌های auth
(`auth-cookies.ts`) از قبل `httpOnly`+`sameSite:'lax'`+`secure` شرطی به
`NODE_ENV==='production'` دارند؛ `.env` واقعی هیچ‌وقت commit نشده
(`git log --all` روی هر دو فایل خالی)؛ محدودیت حجم آپلود آواتار (۲ مگابایت)
از قبل روی `FileInterceptor` اعمال است؛ grep برای secret/token/password
هاردکد در `apps/api/src`+`apps/web/src` چیزی پیدا نکرد.

**تست:** `pnpm lint`/`typecheck`/`build`/`test` کامل مونوریپو سبز (۸/۸/۵/۸
تسک، ۴۰۰/۴۰۰ تست `apps/api` بدون تغییر — یعنی ارتقای multer/sharp هیچ
رگرسیونی در پوشش تست موجود ایجاد نکرد).
...
| بازبینی امنیتی                            | `apps/api/src/main.ts`, `package.json`ی ریشه و `apps/api`                 | ✅ تکمیل — اثبات زنده (بالا)                 |

**فاز ۱۱ اکنون کاملاً تکمیل است.**
