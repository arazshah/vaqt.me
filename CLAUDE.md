...
**آخرین به‌روزرسانی:** فاز ۸ — چت (Conversations)، بک‌اند کامل
**وضعیت:** فاز ۵ (درخواست‌ها) عملاً کامل؛ فاز ۶ (پیشنهادها) کامل؛ فاز ۸ (چت)
بک‌اند کامل (مشاهده/لیست/ارسال پیام + خواندن خودکار)، Socket.IO و UI باقی‌مانده؛
فاز ۷ (AI) هنوز در انتظار
...
| ۸ — چت             | 🔄 در حال انجام | بک‌اند کامل (مشاهده/لیست/ارسال پیام/خواندن خودکار)؛ Socket.IO و UI باقی‌مانده                      |
...
**باقی‌مانده برای فاز ۸:** Socket.IO gateway برای تحویل بی‌درنگ پیام (طبق
بند ۴ همین فایل: auth کوکی‌محور، با تیکت `POST /auth/ws-ticket` به‌عنوان
جایگزین)، و UI فرانت‌اند (`apps/web`: فهرست گفتگوها + صفحه‌ی thread) —
مطابق الگوی فاز ۶ (بک‌اند اول، بعد UI، بعد real-time/polish).
