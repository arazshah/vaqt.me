...
| PR #۱۷ (هات‌فیکس P0) از مسیر عادی (بدون `--admin`) merge شد                                   | `gh pr view 17 --json state,mergedAt,mergeCommit`                                                                                       | ۲۰۲۶-۰۸-۲۳ | `{"mergeCommit":{"oid":"afa8c4188482910aaebd17a41c94aa94bf47cab6"},"mergedAt":"2026-08-23T12:01:19Z","state":"MERGED"}`                                                                                           |
| PR #۱۶ (برش عمودی فاز ۵) بعد از rebase روی #۱۷ و سبز شدن مجدد CI، از مسیر عادی merge شد       | `gh pr view 16 --json state,mergedAt,mergeCommit`                                                                                       | ۲۰۲۶-۰۸-۲۳ | `{"mergeCommit":{"oid":"453fc3c4870fb01c05943e7cc264d4d9f9f3ea30"},"mergedAt":"2026-08-23T12:17:04Z","state":"MERGED"}`                                                                                           |
| `main` بعد از هر دو merge (`453fc3c` روی `afa8c41`) با Postgres/Redis واقعی محلی کامل سبز است | `git checkout main && git pull` سپس `pnpm install/build/typecheck/lint/test` با `DATABASE_URL`/`REDIS_URL`/اسرار صادرشده مطابق `ci.yml` | ۲۰۲۶-۰۸-۲۳ | build/typecheck/lint: `Tasks: 8 successful, 8 total` هرکدام؛ test: `Test Suites: 42 passed, 42 total` `Tests: 284 passed, 284 total`                                                                              |
