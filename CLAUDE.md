...
| حالت تاریک (dark mode) هنوز هیچ toggle واقعی ندارد                                                        | `next-themes` (که با `sonner` نصب شده بود) حذف شد چون بدون `ThemeProvider` یک وابستگی بلااستفاده بود؛ `packages/ui/src/components/ui/sonner.tsx` اکنون همیشه `theme="system"` را hardcode می‌کند (رفتار فعلی بدون تغییر، چون `next-themes` بدون provider هم قبلاً پیش‌فرض `system` بود). تصمیم محصولی درباره‌ی toggle واقعی (و انتخاب مجدد کتابخانه در صورت نیاز) هنوز باز است.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | فاز ۴ (بدنه) |
...
| ~~`@vaqt/db#build` بدون `^build` (پنجمین شاهد) + race مستقل tsup/eslint (ششمین شاهد)~~                    | ✅ رفع شد در فاز ۴ (بازبینی ۲۰۲۶-۰۸-۲۱، همان روز، در PR بعدی) — CI همین PR که تسک `generate` را سخت‌تر کرد (ردیف بالا) خودش شکست: `@vaqt/db#build`'s override فقط `dependsOn: ["generate"]` داشت، نه `^build`؛ چون قدم جدید `tsc --declaration --emitDeclarationOnly` (همان ردیف بالا) حالا `seed.ts`/`enums.test.ts` را هم می‌بیند و هر دو `@vaqt/shared` را import می‌کنند، روی یک runner واقعاً تازه با خطای `Could not find a declaration file for module '@vaqt/shared'` شکست خورد — این نقص از قبل هم وجود داشت (نه چیزی که همین تغییر ایجاد کرد)، فقط CI همان PR قبلی شانسی زمان‌بندی درستی داشت. رفع: `^build` به `@vaqt/db#build` اضافه شد. برای اثبات، به‌جای یک اجرای تمیز، **۱۳ اجرای پیاپی تمیز** (پاک‌کردن کامل `.turbo`/`dist`/`generated` قبل از هرکدام) انجام شد — یک باگ **دوم و کاملاً مستقل** را هم لو داد: `@vaqt/db:lint` و `@vaqt/db:build` هیچ ترتیبی نسبت به هم ندارند (هیچ‌کدام به دیگری وابسته نیست)، و tsup هنگام build یک کپی موقت bundle-شده از `tsup.config.ts` (مثلاً `tsup.config.bundled_lhv2blfbgq.mjs`) داخل ریشه‌ی همان پکیج می‌نویسد و بعد پاک می‌کند؛ اگر `eslint .` دقیقاً همان لحظه آن دایرکتوری را اسکن کند، فایل را برای خواندن صف می‌کند و با `ENOENT` شکست می‌خورد چون tsup تا آن لحظه پاکش کرده — ۵ شکست از ۱۳ اجرا. رفع با نادیده‌گرفتن الگوی `**/tsup.config.bundled_*.mjs` در `eslint.config.mjs` (دقیقاً مثل نادیده‌گرفتن خودِ `tsup.config.ts`). بعد از هر دو رفع: ۲۰ اجرای پیاپی تمیز `pnpm lint`، صفر شکست.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | —            |
