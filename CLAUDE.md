...
**آخرین به‌روزرسانی:** فاز ۷ — AI (بک‌اند نشست/استخراج ساختاریافته) — PR ۱ merge شد
**وضعیت:** فاز ۵ (درخواست‌ها) عملاً کامل؛ فاز ۶ (پیشنهادها) کامل؛ فاز ۸ (چت)
کامل (بک‌اند + UI + Socket.IO برای تحویل بی‌درنگ)؛ فاز ۷ (AI) در حال انجام —
بک‌اند (نشست AI + استخراج ساختاریافته + آداپتر mock/Anthropic) merge شد،
ویزارد UI باقی‌مانده
...
| ۷ — AI             | 🔄 در حال انجام | بک‌اند کامل (نشست AI، استخراج ساختاریافته با retry/fallback، آداپتر mock+Anthropic)؛ ویزارد UI باقی‌مانده |
...
**باقی‌مانده برای فاز ۷:** ویزارد UI در `apps/web` (چت مرحله‌به‌مرحله +
پیش‌نمایش زنده که با `RequestForm` موجود فاز ۵ پر می‌شود) — یک PR جدا،
مطابق الگوی فازهای قبل.
