...
- **صفحه‌ی اصلی هنوز یک لندینگ واقعی نیست.** `apps/web/src/app/page.tsx` هنوز
  همان کارت نمایشی کامپوننت‌های فاز ۴ است (کشف‌شده در بازبینی SEO فاز ۱۰):
  متن «فاز ۴ — سیستم طراحی» را به بازدیدکننده‌ی واقعی نشان می‌دهد و دکمه‌ی
  «شروع کنید» هیچ `href`/handler‌ای ندارد. متادیتای سطح سایت (`layout.tsx`،
  عنوان/توضیح فارسی + `metadataBase`) از قبل درست است، پس SEO/PWA چیزی را
  نمی‌شکند — فقط یعنی محتوای واقعی بازاریابی (hero، کپی، CTA) هنوز نوشته
  نشده. عمداً در کار SEO/PWA فاز ۱۰ دست‌نخورده ماند چون بازطراحی محتوا یک
  تصمیم محصولی جداست، نه پیامد جانبی زیرساخت SEO.
